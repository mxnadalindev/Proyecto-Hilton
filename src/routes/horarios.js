...
router.get('/', loginRequerido, async (req, res) => {
  ...
  `, [fecha]);
  const eventos = await db.all2("SELECT id,nombre,fecha FROM eventos ORDER BY fecha DESC LIMIT 20");
  const porTurno = { manana:[], tarde:[], noche:[] };
  horarios.forEach(h => { if(porTurno[h.turno]) porTurno[h.turno].push(h); });
  res.render('horarios', { horarios, porTurno, fecha, eventos, TURNOS, totalAsignados: horarios.length });
});
...
router.post('/generar', loginRequerido, async (req, res) => {
  ...
  const personal = await db.all2(`
    SELECT id,nombre,puesto FROM usuarios WHERE activo=1
    ORDER BY CASE puesto
      ...
      WHEN 'Panadero' THEN 7 ELSE 8 END, nombre
  `);
