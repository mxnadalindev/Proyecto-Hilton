...
const HORAS_AM = ['4','4.30','5','5.30','6','6.30','7','7.30','8','8.30','9','10','11','12','13'];
const HORAS_PM = ['14','14.30','15','15.30','16','17','18','19','20','21','22'];
const UBICACIONES = ['','C','D','F','B','PF','PB','D8'];
...
function getLunesAnterior(lunes) {
  const d = new Date(lunes + 'T00:00:00');
...
async function getHorarioMap(d0, d6) {
  const rows = await db.all2(
    `SELECT usuario_id, fecha::text, valor FROM horarios_semanales WHERE fecha >= $1 AND fecha <= $2`,
    [d0, d6]
  );
  const map = {};
  rows.forEach(h => {
    if (!map[h.usuario_id]) map[h.usuario_id] = {};
    map[h.usuario_id][h.fecha] = h.valor;
  });
  return map;
}
...
router.get('/', loginRequerido, async (req, res) => {
  ...
  const horariosMap  = await getHorarioMap(dias[0], dias[6]);

  const porSector = {};
  empleados.forEach(e => {
    const s = e.departamento || 'Sin sector';
  ...
  SECTORES.forEach(sector => {
    ...
    dias.forEach(dia => {
      ...
      if (!tieneAlguien) {
        ...
        const nd = ['Dom','Lun','Mar','Mié','Jue','Vie','Sáb'][fecha.getDay()];
        alertas.push({ sector, dia, nombreDia: nd });
      }
    });
  });

  const lunesAnterior   = getLunesAnterior(lunes);
  const diasAnterior    = getDiasSemana(lunesAnterior);
  const horariosAnterior = await getHorarioMap(diasAnterior[0], diasAnterior[6]);

  res.render('horarios', {
    path: 'horarios', lunes, dias, porSector, horariosMap,
    horariosAnterior, diasAnterior,
    SECTORES, ESTADOS, HORAS_AM, HORAS_PM, UBICACIONES, alertas,
    lunesAnterior,
    semanaSiguiente: getLunes(new Date(lunes + 'T00:00:00').setDate(new Date(lunes + 'T00:00:00').getDate() + 7) && new Date(new Date(lunes + 'T00:00:00').setDate(new Date(lunes + 'T00:00:00').getDate() + 7)).toISOString().split('T')[0])
  });
});
...
// ── POST /generar-semana — guardar semana completa ─────
router.post('/generar-semana', loginRequerido, async (req, res) => {
  const { lunes, datos } = req.body; // datos: [{uid, valor}]
  const dias = getDiasSemana(lunes);
  try {
    for (const { uid, valor } of datos) {
      for (const dia of dias) {
        if (!valor || valor.trim() === '') {
          await db.run2('DELETE FROM horarios_semanales WHERE usuario_id=$1 AND fecha=$2', [parseInt(uid), dia]);
        } else {
          await db.run2(`
            INSERT INTO horarios_semanales (usuario_id, fecha, valor)
            VALUES ($1, $2, $3)
            ON CONFLICT (usuario_id, fecha) DO UPDATE SET valor = $3
          `, [parseInt(uid), dia, valor.trim().toUpperCase()]);
        }
      }
    }
    res.json({ ok: true });
...
router.post('/copiar-semana', loginRequerido, async (req, res) => {
  ...
  try {
    const lunesOrigen  = getLunesAnterior(lunes_destino);
...
router.get('/excel', loginRequerido, async (req, res) => {
  ...
  titulo.value = `HORARIOS — HILTON BUENOS AIRES — Semana ${dias[0]} al ${dias[6]}`;
  ...
  const colores = { OFF:'FFFBBF24',VAC:'FFEF4444',RECOFF:'FF22C55E',FERIADO:'FFEF4444',LICENCIA:'FFEC4899',CUMPLE:'FFA855F7',MUDANZA:'FFFB923C' };
  const colSector = ['FFDBEAFE','FFECFDF5','FFFEFCE8','FFFFF7ED','FFFDF4FF','FFF0FDF4','FFEFF6FF','FF334155','FFFFF7F0','FFEEF2FF','FFF7FEE7','FFFEF9C3'];
  let sectorActual = null; let sIdx = -1;

  empleados.forEach(emp => {
    if (emp.departamento !== sectorActual) {
      sectorActual = emp.departamento; sIdx++;
      const sRow = ws.addRow([emp.departamento || 'Sin sector']);
      ws.mergeCells(`A${sRow.number}:J${sRow.number}`);
      sRow.getCell(1).font = { bold: true, size: 10, color: { argb: sectorActual === 'Nocturno' ? 'FFFFFFFF' : 'FF1E3A5F' } };
      sRow.getCell(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: colSector[sIdx % colSector.length] } };
      sRow.getCell(1).alignment = { horizontal: 'center' };
      sRow.height = 18;
    }
    const fila = [emp.nombre, emp.departamento || '', ...dias.map(d => horariosMap[emp.id]?.[d] || '')];
    ...
    row.eachCell((c, col) => {
      ...
      if (col > 2) {
        ...
        if (val && colores[val]) { c.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: colores[val] } }; c.font.bold = true; }
      }
    });
  });
  ...
  res.setHeader('Content-Disposition', `attachment; filename=horarios_${lunes}.xlsx`);
